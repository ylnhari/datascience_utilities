def horizontal_bar_plot_grouped_for_two_columns_annotate_bars_after_scaling(df, main_column, num_columns1, num_column2):
    ...
    df = pd.DataFrame(columns = [Main_Col, Num_Col1, NumCol2],
                      rows = [['A', 50, 5]
                              ['B', 99, 18]
                              ['C', 78, 3]])
    Plot
    ...
    df_melted = pd.melt(df, id_vars=main_column, var_name='col_names', value_name='col_values')
    ...
    if df_melted[~mask].col_values.mean() >= df_melted[mask].col_values.mean():  # pallets mean > loads mean
        smaller_col = num_columns1
        bigger_col = num_column2
    else:
        bigger_col = num_columns1
        smaller_col = num_column2
    ...
    g = seaborn.barplot(y=main_column, x='col_values', hue='col_names', data=df_melted, ax=ax1,
    ...
    for i in range(0, total_bars):
        ...
        g.text(x, y, value, ha='centre', va='centre')
